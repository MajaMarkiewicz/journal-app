...
test.describe('Journal Page', () => {
  ...
  test.skip('#2 Given logged in user on journal, when clicks new entry, fills the form and clicks submit, then new entry is added to database, he is redirected to journal page, and change is visible on journal page', async ({
    page,
  }) => {
    // GIVEN
  ...
  test.skip('#3 Given logged in user on journal page with entries, when clicks an entry, change sth in the form and clicks submit, then entry is updated in database and on journal page', async ({
    page,
  }) => {
    // GIVEN
    // todo mock that user is logged in
